    ...
    st.markdown(f"""
        <div style="background: white; padding: 20px; border-radius: 10px; border: 1px solid #f1f5f9; height: 100%;">
            <div style="display: flex; justify-content: space-between; align-items: flex-start; margin-bottom: 15px;">
                <div><div style="color: #64748b; font-size: 0.85rem; font-weight: 500;">{title}</div></div>
                <div style="color: {color};"><svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">{icon_svg}</svg></div>
            </div>
...
def render_detailed_group_card(title, value, color, icon_key, lines):
    icon_svg = SVG_ICONS.get(icon_key, SVG_ICONS["box"])
    details_html = "".join([f'<div style="display: flex; justify-content: space-between; margin-top: 6px; font-size: 0.75rem;"><span style="color: #94a3b8;">{l[0]}:</span><span style="color: #1e293b; font-weight: 700;">{l[1]}</span></div>' for l in lines])
    st.markdown(f"""
        <div style="background: white; padding: 18px; border-radius: 12px; border: 1px solid #e2e8f0; height: 185px; margin-bottom: 10px;">
            <div style="display: flex; align-items: center; gap: 8px; margin-bottom: 10px;">
                <div style="color: {color};"><svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">{icon_svg}</svg></div>
                <div style="color: #64748b; font-size: 0.8rem; font-weight: 600;">{title}</div>
            </div>
            <div style="font-size: 1.4rem; font-weight: 800; color: #1e293b; margin-bottom: 8px; border-bottom: 1px solid #f1f5f9; padding-bottom: 8px;">{value}</div>
            {details_html}
        </div>
    """, unsafe_allow_html=True)
...
def render_category_distribution(name, count, amount, percentage, color):
    st.markdown(f"""
        <div style="display: flex; align-items: center; justify-content: space-between; padding: 12px; background: #f8fafc; border-radius: 8px; margin-bottom: 8px; border-left: 4px solid {color};">
            <div style="flex: 1;"><div style="font-weight: 600; color: #1e293b; font-size: 0.9rem;">{name}</div><div style="color: #64748b; font-size: 0.8rem;">{count} ед. — {amount} ₽</div></div>
    ...
    st.markdown(f"""
        <div style="background: white; padding: 24px; border-radius: 12px; border: 1px solid #e2e8f0;">
            ...
            <div style="display: flex; flex-direction: column; gap: 12px; font-size: 0.9rem;">
                <div style="display: flex; justify-content: space-between;"><span style="color: #64748b;">Тренд (3 мес):</span><span style="font-weight: 700;">{d.get('trend', 0)} шт/мес</span></div>
                <div style="display: flex; justify-content: space-between; margin-top: 10px; padding-top: 16px; border-top: 2px solid #f1f5f9; color: #1e3a8a;">
                    <span style="font-weight: 800;">ИТОГО НАДО:</span><span style="font-weight: 900; font-size: 1.2rem;">{d.get('total', 0)}</span>
                </div>
            </div>
        </div>
    """, unsafe_allow_html=True)
